import { View, Text, TouchableOpacity, Image, ScrollView, SafeAreaView, Alert, ImageBackground, } from "react-native";
...
const ProfileScreen = (props: ProfileScreenProps) => {
  ...
  return (
    <Container statusBarStyle={'dark-content'} statusBarBackgroundColor={AllColors.white} backgroundColor={AllColors.white}>
      ...
      {/* Header */}
      <CustomHeader type="back" screenName="Profile" onPressBack={() => { props.navigation.goBack(); }} showSearch customImage={Images.BackButton} />
      <ScrollView style={styles.container} contentContainerStyle={styles.scrollContent} showsVerticalScrollIndicator={false}>
        ...
        <ImageBackground source={Images.profileBackground} style={styles.profileSection} resizeMode="cover">
          {/* <View style={styles.profileSection}> */}
          <Animated.View>
            <Image style={{ width: metrics.hp10, height: metrics.hp10 }} resizeMode="contain" source={Images.userAvatar} />
          </Animated.View>
          ...
          <TouchableOpacity style={styles.editButton} onPress={() => props.navigation.navigate("EditProfile")}>
            <View style={styles.editContent}>
              <Animated.View style={styles.editIconWrapper}>
                <Image resizeMode="contain" source={Images.edit} style={styles.editIcon} />
              </Animated.View>
        ...
        <View style={styles.section}>
          {menuItems.map((item) => {
            return (
              <MenuItem title={item.title} subtitle={item.subtitle} iconName={item.iconName} onPress={item.onPress} />
            );
          })}
        </View>
        {/* Stocks Section */}
